...
#I used filter and group_by to sort for all the responses that fall into the 4 respective acreage categories (do I want to eventually divide this into below 100 or 200 acres or above??? like I will do with NWOS?). The position = position_stack line of code addresses the order of responses in my visualization so that the colors line out the way I want them to. Scale_y_continuous is to change percent to 0-100 scale and remove the weird spaces between top and bottom of ggplot. The theme could be taken out or changed to be more custom. The coord_flip turns all the plots to be horizontal instead of vertical and the scale_fill_manual uses the colors I selected in the code chunk above. 
```
...
Q8_acreage_df %>% 
  ...
  coord_flip() +
  scale_fill_manual(values = EmmaColors)
#Emma - this is the original code for Q8 acreage and the code chunk above is altered for sorting/visualization purposes - 4/15/2024
